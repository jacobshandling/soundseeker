...
class EditSuiteView extends React.Component {
  ...
  render() {
    ...
    const blobOptions = this.renderBlobOptions(
      suite.blobs,
      this.props.userBlobMap
    );
    return (
      <section id="edit-suite" className="action-view">
        ...
        >
          <label htmlFor="new-blob-associations">
            Blobs of this Suite (at least 1):
          </label>
          {blobOptions}
        </fieldset>
        <div className="form-group">
          <button
            ...
            onClick={this.props.onEditSuite}
          >
            Submit
          </button>
        </div>
        <button className="btn btn-danger" onClick={this.props.onDeleteSuite}>
          Delete "{suite.name}"
        </button>
      </section>
    );
