...
class EditBlobView extends React.Component {
  ...
  render() {
    ...
    const clipOptions = this.renderClipOptions(
      blob.clips,
      this.props.userClipMap
    );
    return (
      <section id="edit-blob" className="action-view">
        ...
        <fieldset
          ...
          id="new-clip-associations"
        >
          <label htmlFor="new-clip-associations">
            Clips of this blob (at least 1):
          </label>
          {clipOptions}
        </fieldset>
        <div className="form-group">
          <button
            ...
            onClick={this.props.onEditBlob}
          >
            Submit
          </button>
        </div>
        <button className="btn btn-danger" onClick={this.props.onDeleteBlob}>
          Delete "{blob.name}"
        </button>
      </section>
    );
